...
class TestHistoryEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_clear_history_endpoint(self, client: AsyncClient):
        """Test clear history endpoint exists and responds"""
        response = await client.delete("/history/clear-all")
        assert response.status_code in [200, 401]
        if response.status_code == 200:
            assert response.json().get("message") == "Prediction history cleared"
